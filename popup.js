...
function sync(){
  ...
  if(animeListings.length == 0 && rss.length > 0) {
    for(feed of rss) {
      ...
      for(title of anime) {
        var results = getObjects(objects, 'title', title);
  ...
  for(title of anime) {
    var results = getObjects(animeListings, 'title', title);
    $(results).each(function(i, object){
      var imgString = '';
      if(thumbnails){
        var thumb = getObjects(thumbnails, 'title', title);
        if(thumb.length > 0){
          imgString = '<img class="anime-thumbnail" src="'+ thumb[0].data +'" />';
        }
      }
      $('#main').append('<div class="anime-block">'+ imgString +'<div data-title="'+ object['title'] +'" data-link="'+ object['link'] +'" class="result"> <div class="info-title">'+ object['title'] +'</div> <div class="anime-outputs"> <input type="text" class="info-label" value="'+ title +' (A)"><div class="icon clipboard-title-copy"><i class="fa fa-copy"></i></div></div> <input type="text" class="info-link" value="'+ object['link'] +'"><div class="icon clipboard-magnet-copy"><i class="fa fa-copy"></i></div></div></div>');
    });
  }
...
function reloadAnime(){
  ...
  if(anime.length > 0){
    for(title of anime){
      $('#anime-info').append('<div data-title="'+ title +'" class="anime-title"> '+ title +' <div class="icon remove-anime"><i class="fa fa-times"></i></div></div>');
    }
  }
}
